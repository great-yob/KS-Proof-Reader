...
### 첫 명령으로 자동 승인 잠금을 건다 (사용자 지시 2026-07-31)

"완주 권한"은 실행 흐름의 원칙일 뿐, 그것만으로는 도구 승인 창이 사라지지 않는다.
릴리스는 임의의 `python -c`·`gh`·`git`·빌드 명령을 수십 번 돌리므로 사용자가 그때마다
Yes를 눌러야 했다. 그래서 **잠금 파일이 있는 동안만** 도구 호출을 자동 승인하는
PreToolUse 훅을 둔다(`.claude/hooks/release-autoapprove.sh`, 배선은
`.claude/settings.local.json`).

**릴리스를 시작하면 다른 무엇보다 먼저 이 명령을 실행한다** — 토씨 하나 바꾸지 말 것
(허용 규칙이 이 문자열과 정확히 일치할 때만 승인 없이 통과한다):

```bash
touch .claude/.release-autoapprove
```

그리고 릴리스가 끝나거나 **점검 실패로 중단할 때** 반드시 해제한다:

```bash
rm -f .claude/.release-autoapprove
```

안전장치는 이중이다 — 잠금이 **2시간**보다 오래되면 훅이 무시하고, **턴이 끝나면**
Stop 훅이 파일을 지운다. 그래도 해제를 스킬의 책임에서 빼지 말 것: 자동 승인이
켜진 채로 다른 작업이 이어지는 상황을 만들지 않는 게 이 잠금의 존재 이유다.

⚠ 잠금은 **승인 창만** 없앤다. 하지 말 것 목록(설치 파일 실행·게이트 우회 등)은
그대로 유효하며, 오히려 사용자가 중간에 막아 줄 수 없으므로 더 엄격히 지킨다.
...
## 3-b. 설치 파일 백업 — 빌드 직후 즉시 (사용자 지시 2026-08-05)

**방금 만든 설치 파일을 `Work Utility\백업\`에 복사한다.** 업로드보다 먼저, 빌드가
끝나자마자 한다.

```bash
cp "dist/release/KS-AI-Editor-Setup-{ver}.exe" "/c/Users/user9/Desktop/Work Utility/백업/"
ls -la "/c/Users/user9/Desktop/Work Utility/백업/" | grep Setup   # 침묵 검사 — 없을 때만 보고
```

⚠ **이 백업을 "구버전을 지우기 직전"으로 미루지 말 것.** 예전 절차가 그랬고, 그래서
4-b에서 "백업에 없네? → GitHub에서 194MB를 다시 내려받아 채우기"라는 우회를 매번
하게 됐다(2026-08-04 v1.5.2에서 실제 발생). 백업이 확실히 가능한 시점은 **파일이 손에
있는 지금뿐**이다 — 릴리스를 지운 뒤엔 GitHub에서 받을 수도 없다.

또한 `build_dist.py --clean`은 `dist/`를 통째로 지운다. **다음 릴리스를 빌드하는 순간
이번 설치 파일은 로컬에서도 사라진다** — 지금 복사하지 않으면 되돌릴 방법이 GitHub
하나로 줄고, 그 GitHub 릴리스마저 다음 릴리스 때 4-b가 지운다.

app.zip·data.zip은 백업하지 않는다. 되돌아갈 때 필요한 건 **최초 설치 파일**이고,
업데이트 자산은 그 위에서 다시 만들 수 있다.
...
⚠ **삭제 전에 그 버전의 설치 파일이 `Work Utility\백업\`에 있는지 확인**한다. 지우고 나면
GitHub에서 다시 받을 수 없다 — 새 버전에 결함이 드러났을 때 **되돌아갈 배포본이
그것뿐**이고, 신규 사용자에게 건넬 수단도 사라진다.

3-b를 지킨 릴리스라면 **이미 들어 있다**(그 버전을 릴리스할 때 넣었으므로). 확인은
1초짜리 존재 검사이지 다운로드 절차가 아니다:

```bash
ls "/c/Users/user9/Desktop/Work Utility/백업/KS-AI-Editor-Setup-{옛버전}.exe"
```

없으면(3-b 도입 전에 나간 옛 릴리스) **삭제 전에** 내려받아 채운다 — 이건 레거시
폴백이며, 매 릴리스마다 하는 정상 경로가 아니다:

```bash
gh release download v{옛버전} --pattern "*Setup*.exe" --dir "/c/Users/user9/Desktop/Work Utility/백업"
```
...
- **자동 승인 잠금을 해제한다**(성공이든 중단이든 예외 없이):
  ```bash
  rm -f .claude/.release-autoapprove
  ```
...
- **설치 파일 백업을 "지우기 직전"으로 미루기** — 백업은 **빌드 직후**(3-b)에 한다.
  미루면 4-b에서 GitHub 재다운로드로 때우게 되고(2026-08-04 실제 발생), `--clean`이나
  릴리스 삭제가 먼저 오면 그마저 불가능해진다.
- **구버전 설치 파일이 백업에 없는데 릴리스를 삭제** — 결함이 드러났을 때
  되돌아갈 배포본이 없어진다. 존재를 확인하고, 없으면 받아서 채운 뒤 지운다.
...
- **자동 승인 잠금을 남긴 채 끝내기** — 성공·실패·중단 어느 경로든 마지막에
  `rm -f .claude/.release-autoapprove`. 남겨 두면 이어지는 작업까지 무승인으로 돈다
  (2시간 만료·Stop 훅이 받쳐 주지만 그건 백스톱이지 절차가 아니다).
- **잠금을 릴리스 아닌 작업에 쓰기** — 이 훅은 `/release` 전용이다. 다른 작업이
  느리다고 잠금을 걸지 말 것.
