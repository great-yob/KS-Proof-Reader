...
_DROP = (
    ...
    "등 컨트롤", "그중 실제 각주",
    # ── 이음매 정합 개별 판정 — '[보완] 용어간 이음매 정합성 판정 n건 추가' 한 줄에
    #    흡수된다(사용자 지정 2026-07-30). ⚠ 요약 라인의 '이음매 정합성'과 겹치지 않게
    #    개별 라인의 선두 '· '까지 포함해서 적을 것.
    "· 미탐 보완", "· 방향 정정", "· 방향 오판 카드 제거",
    "· 이음매 그룹", "· 이음매 정합 '",
    #    '정당한 구분 유지'는 카드를 만들지 않는 **보존** 판정이라 위 요약(n)에 안 들어간다
    #    → 자기 요약 라인 '[보완] 정당한 구분 유지 n건 추가'로 따로 접는다(사용자 지정
    #    2026-07-31). 개별 라인은 진단용으로 원문 로그에 그대로 남는다.
    "· 정당한 구분 유지",
)
