...
(사용자 지적 2026-07-23). 그래서 **표시 계층에서만** 세 가지를 적용한다:
  ① 저정보 라인 숨김(_DROP·_ITEM_RE)      — 배관/설명/개별 항목
  ② 핵심 키워드 요약(_RULES)              — 사유 괄호·부연 절 제거, 수치는 보존
  ③ 반복 진행 라인 합침(coalesce key)      — 'AI 분석 n/m 청크'는 한 행이 갱신

⚠ **화면 로그의 항목 표기 규약(사용자 지정 2026-07-31 — 새 로그를 더할 때도 지킬 것).**
  · 개별 예시('원문' → '교정' 같은 낱말 나열)는 **화면에 절대 내지 않는다.** 진단이
    필요하면 워커/엔진이 `      · …` 들여쓴 **불릿 라인**으로 원문 로그에만 남긴다
    (_ITEM_RE가 선두 불릿만 보고 통째로 숨긴다 — 문구별 _DROP 등록 불필요).
  · 화면에 낼 것은 **집계뿐**이다 — 반드시 `n건` 형태의 수치를 담는다.
  · 모든 표시 라인은 선두에 관련 `[태그]`를 단다. 태그 없는 라인은 만들지 않는다.
  ⇒ 요컨대 **불릿 = 원문 로그 전용 상세, `[태그] … n건` = 화면**이라는 2층 규약이다.
...
_DROP = (
    ...
    "문서가 큽니다",
    # ── 각주 분류 집계(사용자 지정 2026-07-30) — 미리보기 [각주] 표지로 이미 보인다
    "등 컨트롤", "그중 실제 각주",
    # ⚠ 이음매 정합·본문 대조 등의 **개별 판정 라인은 여기 적지 않는다** — 선두 불릿
    #   규약(_ITEM_RE)이 문구와 무관하게 전부 숨긴다. 새 상세 로그를 문구별로
    #   등록하다 빠뜨리는 사고를 막으려고 2026-07-31에 규약으로 일반화했다
    #   ('상호 무효화 쌍 → 사용자 결정 …'이 미등록이라 화면에 샜다).
)

# 개별 항목(상세) 라인 — 선두가 불릿이면 **문구 불문** 화면에서 숨긴다.
#   원문 로그에는 남으므로 진단은 그대로 가능하고, 화면에는 부모 요약 라인의
#   '…n건'만 나간다(위 docstring의 항목 표기 규약). 집계 라인은 불릿을 쓰지 말 것 —
#   워커/윈도우는 요약에 '→ '나 '[태그]'를 쓴다.
...
# ⚠ 그리고 내용에는 **개별 예시가 아니라 `n건` 집계**만 담는다(사용자 지정
#   2026-07-31). 낱말을 보여 주고 싶으면 규칙을 만들지 말고 워커 쪽에서 불릿
#   상세로 내보낼 것 — 화면은 _ITEM_RE가 가리고 원문 로그에만 남는다.
_RULES = tuple((re.compile(p), r, k) for p, r, k in (
    ...
    # ── 사전 스크리닝 ──────────────────────────────
    (r"^강제 줄 나눔 ([\d,]+)곳 복원.*$",             r"[추출] 강제 줄 나눔 \1곳 복원", ""),
    ...
    # ('(이하 …)' 같은 예시 표기는 화면에서 뺀다 — 규약상 집계만)
    (r"^저자 선언 약칭 ([\d,]+)건 검수 카드 제외.*$",  r"[검증] 저자 선언 약칭 \1건 제외", ""),
    (r"^한자 병기 독음 ([\d,]+)건 검수 카드 제외.*$",  r"[검증] 한자 병기 독음 \1건 제외", ""),
    # 억제 '사유별 내역'은 괄호에 담겨 원문 로그에만 남고 화면엔 합계만 나간다.
    #   이 라인 자체를 숨기지 말 것 — 미등재어가 어디서 사라졌는지 보여주는 유일한 창이다.
    (r"^미등재어 검수 카드 억제 ([\d,]+)건.*$",       r"[검증] 검수 카드 억제 \1건", ""),
    (r"^안전망 재개방 ([\d,]+)건.*$",                r"[검증] 안전망 재개방 \1건 복구", ""),
    (r"^적용 정합성: 조사 변형 교정 ([\d,]+)건.*$",    r"[검증] 조사 변형 \1건 제외", ""),
    (r"^교정 합성: .*?([\d,]+)건을 한 카드로.*$",      r"[검증] 교정 합성 \1건 통합", ""),
    (r"^괄호 뒤 조사 받침 호응 보정 ([\d,]+)건.*$",    r"[검증] 괄호 뒤 조사 보정 \1건", ""),
    (r"^\[문장부호\] 따옴표 방향 반대 ([\d,]+)건.*$",   r"[문장부호] 따옴표 방향 \1건 검수", ""),
    (r"^신뢰도 낮음 ([\d,]+)건은 자동 적용에서 제외.*$", r"[검증] 저신뢰 \1건 자동 적용 제외", ""),
    # (개별 충돌 라인은 워커가 불릿 상세로 내보내 _ITEM_RE가 숨긴다 — 아래 집계만 표시)
    ...
    # ── 업데이트(확인·설치) ────────────────────────
    #   core/updater.py는 GUI를 모르므로 태그 없이 '✔/✗ …'만 찍는다 → 태그는 여기서 단다.
    (r"^새 버전 (.+?) 확인.*$",                      r"[업데이트] 새 버전 \1 확인", ""),
    (r"^작업이 끝난 뒤 사이드바.*$",                  "[업데이트] 작업 종료 후 설치 가능", ""),
    (r"^[✔✓]\s*데이터 설치 완료\(([\d,]+)\s*단어\).*$", r"[업데이트 완료] 사전 데이터 \1단어 설치", ""),
    (r"^[✔✓]\s*업데이트 준비 완료.*$",                "[업데이트 완료] 앱 종료 시 자동 교체", ""),
    (r"^[✗✕]\s*데이터 무결성 실패.*$",                "[업데이트 오류] 데이터 무결성 실패 · 설치 취소", ""),
    (r"^[✗✕]\s*(?:데이터|앱) 패키지.*$",              "[업데이트 오류] 패키지 구성 오류 · 설치 취소", ""),
    (r"^[✗✕]\s*(?:데이터 설치|앱 업데이트) 실패.*$",   "[업데이트 오류] 설치 실패", ""),
    (r"^[✗✕]\s*개발 환경에서는.*$",                   "[업데이트] 개발 환경 · 자동 설치 안 함", ""),
