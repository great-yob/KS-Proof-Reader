...
**2b. 사전 전용 "검수 모드"** ⚠ **아래 진입 경로·불변식은 폐기됨(2026-08-05) — 역사적 기록.**
> **현행**: '검수 모드'는 설정의 **결과물 축**(`SetupPanel` → 교정본 + 정오표 / **정오표만**)으로
> 사용자가 직접 고른다(`options["errata_only"]`). 아래 두 서술은 더 이상 사실이 아니다:
> - ~~"AI scope 0개 선택 시 자동 진입"~~ — `scopes_selected()`가 분석 시작을 막고 윤문은
>   잠겨 있어 **현행 UI에서 도달 불가능**하다. 그 자리는 별개 옵션인 'AI 분석 제외'(`_no_ai`)가
>   대신하는데, 그건 Gemini 호출만 빼고 적용·정오표는 그대로 수행한다(HWP 미수정이 아니다).
> - ~~"HWP를 열지도 수정하지도 않음"~~ — **열지는 않는다는 부분이 폐기됐다.** 정오표의 쪽
>   번호를 읽으려면 문서를 열어야 한다(찾기만·`save_as` 없음 — 수정하지 않는다는 원래 취지는
>   그대로). 상세는 CLAUDE.md '결과물 (output) is a user-chosen axis' 절.
>
> ⚠ 예전엔 이 분기가 '수락한 치환이 0건'일 때 **우연히** 도달하는 상태였고, 그래서 의도한
> 산출물인지 사고인지 구분할 수 없었다. 그 상태(=적용할 게 없는 실행)는 지금도 존재하지만
> 이제 옵션과 **다른 것**으로 취급된다.
- (구) AI scope 0개 선택 시 자동 진입. 푸터 버튼이 "사전 검수 시작 (AI 미사용)"으로 바뀜.
