...
# ══════════════════════════════════════════════════════════
# ▌㉙ 한자 병기 괄호 안의 한글 = 저자가 단 '독음'이지 어휘가 아니다
# ══════════════════════════════════════════════════════════
# 사용자 보고 2026-08-05(실파일: 中國科學院 보고서). 중국 인명·기관명을 한자로 쓰고
# 괄호 안에 한글 독음을 병기한 원고에서, 그 **괄호 안 한글이 전부 '어느 사전에도 없음'
# 검수 카드**가 됐다 — `戴彧虹(대욱홍)` → 카드 '대욱홍'. 실측: 이 문서 병기 163곳
# (149종) 중 **71건**이 그대로 dict_flag가 됐다(전체 225건의 32%).
#
# ⚠ 이건 사전을 넓혀서 풀 문제가 아니다 — 중국 인명 독음은 닫히지 않는 집합이고,
# 우리말샘·온용어에도 없다([[linebreak-loss-and-propernoun-limits]] 폐쇄 ⓑ와 같은 부류).
# 대신 **모양이 정체를 확정한다**: 한자 런 **바로 뒤** 괄호 안에 든 순수 한글 런은
# 그 한자의 독음·음역이다. 저자가 방금 원어를 적어 두었으므로 표제어 확인이 무의미하다.
# (예외를 열거하지 않는 '모양 판정자' — [[bracket-and-dependent-noun-spacing]] ⑨ 원칙.)
#
# ⚠ **방향이 중요하다.** 막는 건 `漢字(한글)` 하나뿐이다. 흔한 `한글(漢字)`
# (개혁개방(改革開放))에서는 한글이 본문이므로 **반드시 계속 검사해야 한다** — 실제로
# 이 문서의 '과교융합(科教融合)'은 이 가드에 걸리지 않고 카드로 남는다(의도).
# ⚠ 길이 일치(한자 n자 = 한글 n음절)는 **요구하지 않는다** — 실측상 중국어 발음 표기
# (北京大学(베이징대학) 4:5)·부분 병기(中国科技网(과기망))가 흔해 3:3만 받으면 절반이 샌다.
_HANJA_CH = r"㐀-䶿一-鿿豈-﫿"
_HANJA_GLOSS_RE = re.compile(
    r"[" + _HANJA_CH + r"]\s*[(（]\s*([가-힣]{2,})\s*[)）]")


def hanja_gloss_readings(document_text: str) -> frozenset:
    """한자 뒤 괄호에 병기된 한글 독음 집합. '戴彧虹(대욱홍)' → {'대욱홍'}.

    워커 [6] 사전 안전망이 '검수 필요' 카드를 억제하는 데 쓴다(저자 선언 약칭
    `author_defined_abbreviations`와 같은 자리·같은 성격). AI 입력(의심 단어 힌트)은
    건드리지 않는다 — 억제는 카드 계층에서만 한다.
    """
    if not document_text:
        return frozenset()
    return frozenset(m.group(1) for m in _HANJA_GLOSS_RE.finditer(document_text))
